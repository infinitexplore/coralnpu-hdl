...
/**
 *  A testbench DUT that instantiates a single SecdedEncoder so it can be tested
 *  with cocotb.
 */
class SecdedEncoderTestbench(val w: Int, val moduleName: String)
    extends Module {
  ...
  (new ChiselStage).execute(
    ...
    Seq(
      ChiselGeneratorAnnotation(() =>
        new SecdedEncoderTestbench(p.lsuDataBits, "SecdedEncoderTestbench128")
      )
...
object EmitSecdedEncoderTestbench32 extends App {
  (new ChiselStage).execute(
    ...
    Seq(
      ChiselGeneratorAnnotation(() =>
        new SecdedEncoderTestbench(32, "SecdedEncoderTestbench32")
      )
    ) ++ Seq(FirtoolOption("-enable-layers=Verification"))
  )
}
...
object EmitSecdedEncoderTestbench57 extends App {
  (new ChiselStage).execute(
    ...
    Seq(
      ChiselGeneratorAnnotation(() =>
        new SecdedEncoderTestbench(57, "SecdedEncoderTestbench57")
      )
    ) ++ Seq(FirtoolOption("-enable-layers=Verification"))
  )
}
