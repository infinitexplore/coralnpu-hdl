 ...
 *
 *  Covers register access, data path (loopback), baud rate timing, manual
 *  chip-select control, and asynchronous clock domain crossing.
 */
class SpiMasterSpec extends AnyFreeSpec with ChiselSim {
  val p = new Parameters

  /**
   *  Helper to perform a robust TileLink Put transaction through a specific
   *  clock domain.
   */
  def tlWrite(
  ...
  /**
   *  Helper to perform a robust TileLink Get transaction through a specific
   *  clock domain. Returns (data, error).
   */
  def tlRead(
      tl: OpenTitanTileLink.Host2Device,
      clock: Clock,
  ...
  /**
   *  Wrapper for tlRead to maintain compatibility with existing tests that only
   *  expect data.
   */
  def tlReadData(
      tl: OpenTitanTileLink.Host2Device,
  ...
  "SpiMaster Manual CS" in {
    simulate(new SpiMasterCtrl(p)) { dut =>
      ...
      dut.clock.step(10)
      assert(
        dut.io.spi.csb.peek().litValue == 0,
        "CS should stay asserted in manual mode"
      )

      tlWrite(dut.io.tl, dut.clock, 0x10.U, 0x00.U) // Deassert
      dut.clock.step(10)
      assert(
        dut.io.spi.csb.peek().litValue == 1,
        "CS should stay deasserted in manual mode"
      )
    }
  }
  ...
  "SpiMaster TX Backpressure" in {
    simulate(new SpiMasterCtrl(p)) { dut =>
      ...
      assert(
        ...
        "Host should be stalled when TX FIFO is full"
      )
      dut.io.tl.a.valid.poke(false.B)
    }
  }
  ...
  "SpiMaster RX Overflow Stall" in {
    simulate(new SpiMasterCtrl(p)) { dut =>
      ...
      assert(
        sclk1 == sclk2,
        "SPI FSM should be stalled awaiting RX FIFO availability"
      )
      ...
      assert(
        (status & 2) == 0,
        "Transaction should have finalized after FIFO read"
      )
    }
  }
