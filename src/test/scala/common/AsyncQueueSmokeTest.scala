...
trait WithVcd { this: HasCliOptions =>
  implicit override def backendSettingsModifications
  ...
  implicit override def commonSettingsModifications
      : svsim.CommonSettingsModifications =
    (original: CommonCompilationSettings) => {
      original.copy(
        simulationSettings =
          original.simulationSettings.copy(enableWavesAtTimeZero = true)
      )
    }
}
...
class AsyncQueueSmokeTest extends Module {
  ...
  val queue = Module(
    new AsyncQueue(UInt(32.W), AsyncQueueParams.singleton(safe = false))
  )
...
class AsyncQueueSmokeSpec extends AnyFreeSpec with ChiselSim with WithVcd {
  "AsyncQueueSmokeTest should pass a value across clock domains" in {
    simulate(new AsyncQueueSmokeTest) { dut =>
      ...
      if (timeout == 0) {
        fail(
          s"Timed out after ${deqValidTimeoutCycles} cycles waiting for deq.valid to go high"
        )
      }
