...
class TlulFifoSync_(
  ...
  override val desiredName =
    s"${socketName}_TlulFifoSync_d${reqDepth}r${rspDepth}"
}
...
class TlulSocketM1(
  ...
  val hrsp_fifo_i = Wire(
    Vec(M, Flipped(Decoupled(new OpenTitanTileLink.D_Channel(p))))
  )
  ...
  val arb = Module(
    new CoralNPURRArbiter(
      new OpenTitanTileLink.A_Channel(p),
      M,
      moduleName = Some(s"${moduleName}_CoralNPURRArbiter_${M}")
    )
  )
  for (i <- 0 until M) {
  ...
  io.tl_d.d.ready := (VecInit(
    hrsp_fifo_i.map(_.ready)
  ).asUInt & rsp_arb_grant).orR
  dfifo.io.host.d.ready := (VecInit(
    hrsp_fifo_i.map(_.ready)
  ).asUInt & rsp_arb_grant).orR
}
