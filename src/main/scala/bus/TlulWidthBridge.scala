...
class TlulWidthBridge(val host_p: TLULParameters, val device_p: TLULParameters)
    extends Module {
  ...
  if (hostWidth > deviceWidth) {
    ...
    val d_data_reg     = RegInit(
      VecInit(
        Seq.fill(numHostSources)(VecInit(Seq.fill(ratio)(0.U(deviceWidth.W))))
      )
    )
    val d_resp_reg     = RegInit(
      VecInit(
        Seq.fill(numHostSources)(
          0.U.asTypeOf(new OpenTitanTileLink.D_Channel(host_p))
        )
      )
    )
    val d_valid_reg    = RegInit(0.U(numHostSources.W))
    val d_fault_reg    = RegInit(0.U(numHostSources.W))
    val beats_received = RegInit(
      VecInit(Seq.fill(numHostSources)(0.U(ratio.W)))
    )
    ...
    val aggregated_data = VecInit(
      d_data_reg(active_host_source).zipWithIndex.map { case (d, i) =>
        Mux(
    ...
    wide_resp_wire.error := d_resp_reg(active_host_source).error || d_fault_reg(
      active_host_source
    )
    ...
    io.tl_h.d.valid     := d_valid_reg(
      active_host_source
    ) && req_info_q.io.deq.valid
    io.tl_h.d.bits      := d_gen.io.d_o
    io.tl_h.d.bits.data := full_data

    when(io.tl_d.d.fire) {
      val next_beats_received =
        beats_received(host_source_idx) | (1.U << beat_idx)
    ...
    when(io.tl_h.d.fire) {
      ...
      d_resp_reg(active_host_source)     := 0.U.asTypeOf(
    ...
    val read_mask      =
      (((1.U << size_in_bytes) - 1.U) << address_offset)(hostBytes - 1, 0)
    ...
    val host_beat_idx       =
      io.tl_h.a.bits.address(log2Ceil(hostBytes) - 1, log2Ceil(narrowBytes))
    ...
    val beats    = Wire(
      Vec(ratio, Valid(new OpenTitanTileLink.A_Channel(device_p)))
    )
    ...
    for (i <- 0 until ratio) {
      ...
      val narrow_mask  =
        (effective_mask >> (i * narrowBytes)).asUInt(narrowBytes - 1, 0)
      ...
      narrow_req.source := Cat(
        io.tl_h.a.bits.source,
        beat_source_offset(log2Ceil(ratio) - 1, 0)
      )
    ...
    val addr_lsb_regs  = RegInit(
      VecInit(Seq.fill(numSourceIds)(0.U(addr_lsb_width.W)))
    )
