...
class Spi2TLUL(p: Parameters) extends Module {
  ...
  require(
    kBufferDepth * kBufferWidth / 8 <= 65536,
    "Total buffer size cannot exceed 65536 bytes"
  )
  ...
  val miso_buffer_reg    =
    withClockAndReset(io.spi.clk, spi_domain_reset.asAsyncReset) {
      RegInit(0.U.asTypeOf(Valid(UInt(8.W))))
    }
  val next_miso_byte_reg =
    withClockAndReset(io.spi.clk, spi_domain_reset.asAsyncReset) {
      RegInit(0.U.asTypeOf(Valid(UInt(8.W))))
    }
  ...
  val spi2tlul_q = Module(
    new AsyncQueue(UInt(8.W), AsyncQueueParams(depth = 2, safe = false))
  )
  ...
  val spi_bulk_read_len_reg        =
    withClockAndReset(io.spi.clk, spi_domain_reset.asAsyncReset) {
      RegInit(0.U(16.W))
    }
  val spi_bulk_read_sent_count_reg =
    withClockAndReset(io.spi.clk, spi_domain_reset.asAsyncReset) {
      RegInit(0.U(16.W))
    }

  object SpiCmdState extends ChiselEnum {
    val sIdle, sGotBulkReadAddr, sGotBulkReadLenL, sWaitBulkReadLenH, sSendData,
        sGotOtherCmd = Value
  }
  val spi_cmd_state = withClock(io.spi.clk) { RegInit(SpiCmdState.sIdle) }

  val is_first_byte_reg =
    withClockAndReset(io.spi.clk, io.spi.csb.asAsyncReset) {
      RegInit(true.B)
    }
  ...
  val is_bulk_read_cmd_l =
    is_first_byte && is_write_cmd && (cmd_addr === SpiRegAddress.BULK_READ_PORT_L.asUInt)
  val is_bulk_read_cmd_h =
    !is_first_byte && is_write_cmd && (cmd_addr === SpiRegAddress.BULK_READ_PORT_H.asUInt)

  val next_spi_cmd_state = MuxCase(
    spi_cmd_state,
    Seq(
  ...
  val is_bulk_read_start      =
    byte_received && (spi_cmd_state === SpiCmdState.sGotBulkReadAddr)
  val is_bulk_read_len_h_byte =
    byte_received && (spi_cmd_state === SpiCmdState.sWaitBulkReadLenH)
  ...
  val is_bulk_status_read = is_first_byte && !completed_byte(
    7
  ) && (cmd_addr === SpiRegAddress.BULK_READ_STATUS_REG_L.asUInt || cmd_addr === SpiRegAddress.BULK_READ_STATUS_REG_H.asUInt) && (spi_cmd_state =/= SpiCmdState.sGotOtherCmd)
  ...
  object SpiState extends ChiselEnum {
    val sIDLE, sWAIT_WRITE_DATA, sSEND_READ_DATA, sBULK_WRITE_DATA,
        sBULK_READ_DATA = Value
  }
  ...
  val write_data_buffer   = SRAM(
    kBufferDepth,
    UInt(kBufferWidth.W), /*readPorts=*/ 3, /*writePorts=*/ 1,
    /*readWritePorts=*/ 0
  )
  val read_data_buffer    = withClock(io.spi.clk) {
    SRAM(
      kBufferDepth,
      UInt(kBufferWidth.W), /*readPorts=*/ 1, /*writePorts=*/ 1,
      /*readWritePorts=*/ 0
    )
  }
  val bulk_read_write_ptr =
    withClockAndReset(io.spi.clk, spi_domain_reset.asAsyncReset) {
      RegInit(0.U(log2Ceil(kBufferDepth).W))
    }
  val spi_bulk_read_ptr   =
    withClockAndReset(io.spi.clk, spi_domain_reset.asAsyncReset) {
      RegInit(
        0.U(log2Ceil(kBufferDepth * kBufferWidth / 8).W)
      ) // Byte pointer into the data buffer
    }
  val bulk_write_ptr      = RegInit(
    0.U(log2Ceil(kBufferDepth * kBufferWidth / 8).W)
  ) // Byte pointer for writes
  ...
  val tl_cmd_reg_write =
    do_write && (addr_reg === SpiRegAddress.TL_CMD_REG.asUInt)
  val tl_cmd_reg_data  = spi2tlul_q.io.deq.bits

  val tl_to_spi_bulk_q = Module(
    new AsyncQueue(
      UInt(kBufferWidth.W),
      AsyncQueueParams(depth = 2, safe = false)
    )
  )
  ...
  val tlul2spi_q = Module(
    new AsyncQueue(UInt(8.W), AsyncQueueParams.singleton(safe = false))
  )
  ...
  val state_next = MuxCase(
    ...
    Seq(
      ...
      (spi_state_reg === SpiState.sWAIT_WRITE_DATA && spi2tlul_q.io.deq.fire)                                                        ->
        Mux(
          addr_reg === SpiRegAddress.BULK_WRITE_PORT_H.asUInt,
          SpiState.sBULK_WRITE_DATA,
          Mux(
            addr_reg === SpiRegAddress.BULK_READ_PORT_H.asUInt,
            SpiState.sBULK_READ_DATA,
            SpiState.sIDLE
          )
        ),
  ...
  val writing_addr_reg =
    spi_state_reg === SpiState.sWAIT_WRITE_DATA && spi2tlul_q.io.deq.fire
  for (i <- 0 until 4) {
    tl_addr_reg(i) := Mux(
      writing_addr_reg && (addr_reg === (SpiRegAddress.TL_ADDR_REG_0 + i.U)),
      data,
      tl_addr_reg(i)
    )
  }

  val writing_len_reg_l =
    do_write && addr_reg === SpiRegAddress.TL_LEN_REG_L.asUInt
  val writing_len_reg_h =
    do_write && addr_reg === SpiRegAddress.TL_LEN_REG_H.asUInt
  tl_len_reg := Cat(
    Mux(writing_len_reg_h, data, tl_len_reg(15, 8)),
    Mux(writing_len_reg_l, data, tl_len_reg(7, 0))
  )

  val writing_bulk_write_port_l =
    do_write && addr_reg === SpiRegAddress.BULK_WRITE_PORT_L.asUInt
  val writing_bulk_write_port_h =
    do_write && addr_reg === SpiRegAddress.BULK_WRITE_PORT_H.asUInt
  val writing_bulk_read_port_l  =
    do_write && addr_reg === SpiRegAddress.BULK_READ_PORT_L.asUInt
  val writing_bulk_read_port_h  =
    do_write && addr_reg === SpiRegAddress.BULK_READ_PORT_H.asUInt
  ...
  val write_old_word =
    write_data_buffer.readPorts(1).data // Still combinational for write
  val write_new_word          = (write_old_word & write_mask) | (data << write_shift)
  val write_cmd_fire          = tl_cmd_reg_write && tl_cmd_reg_data === 2.U
  val writing_data_buf_single =
    do_write && addr_reg === SpiRegAddress.DATA_BUF_PORT.asUInt
  val writing_bulk_data       =
    spi_state_reg === SpiState.sBULK_WRITE_DATA && spi2tlul_q.io.deq.fire
  val writing_data_buf        = writing_data_buf_single || writing_bulk_data
  val start_bulk_write        =
    do_write && addr_reg === SpiRegAddress.BULK_WRITE_PORT_H.asUInt
  bulk_write_ptr := Mux(
    write_cmd_fire || start_bulk_write,
    0.U,
  ...
  val byte_index    =
    RegNext(RegNext(spi_bulk_read_ptr(byte_index_bits - 1, 0), 0.U), 0.U)
  ...
  val read_map = Seq(
    ...
    SpiRegAddress.TL_STATUS_REG.asUInt       -> MuxLookup(
      tl_read_state_reg.asUInt,
      0.U
    )(status_map),
    SpiRegAddress.TL_WRITE_STATUS_REG.asUInt ->
      MuxLookup(tl_write_state_reg.asUInt, 0.U)(write_status_map),
    SpiRegAddress.DATA_BUF_PORT.asUInt       -> (selected_word.asUInt >> (byte_index << 3.U))(
      7,
      0
    )
  )
  ...
  val reading_bulk_data =
    spi_state_reg === SpiState.sBULK_READ_DATA && tlul2spi_q.io.enq.fire
  val start_bulk_read   =
    do_write && addr_reg === SpiRegAddress.BULK_READ_PORT_H.asUInt

  bulk_count_reg := Mux(
    start_bulk_write || start_bulk_read,
    0.U,
    Mux(
      writing_bulk_data || reading_bulk_data,
      bulk_count_reg + 1.U,
      bulk_count_reg
    )
  )
  ...
  withClock(io.spi.clk) {
    ...
    val reading_bulk_data_byte =
      spi_cmd_state === SpiCmdState.sSendData && byte_received
    ...
    bulk_read_write_ptr                    := Mux(
      tl_to_spi_bulk_q.io.deq.fire,
      bulk_read_write_ptr + 1.U,
      bulk_read_write_ptr
    )

    spi_bulk_read_ptr := Mux(
      reading_bulk_data_byte,
      spi_bulk_read_ptr + 1.U,
      spi_bulk_read_ptr
    )

    val reset_sent_count =
      spi_cmd_state === SpiCmdState.sGotBulkReadAddr && byte_received
    spi_bulk_read_sent_count_reg := Mux(
      reset_sent_count,
      0.U,
      Mux(
        reading_bulk_data_byte,
        spi_bulk_read_sent_count_reg + 1.U,
    ...
    val selected_read_byte =
      (selected_read_word >> (read_byte_index_reg << 3.U))(7, 0)

    // --- MISO Path Refactor with Forwarding ---

    // 1. Define the single source of new data and its validity
    val selected_status_byte   = Mux(
      cmd_addr === SpiRegAddress.BULK_READ_STATUS_REG_L.asUInt,
      bulk_read_bytes_available(7, 0),
      bulk_read_bytes_available(15, 8)
    )
    val miso_data_source_bits  = MuxCase(
      0.U,
      Seq(
    ...
    val miso_data_source_valid =
      is_bulk_read_start || reading_bulk_data_byte || is_bulk_status_read || tlul2spi_q.io.deq.fire
    ...
    val shifter_valid_source =
      Mux(miso_data_source_valid, true.B, next_miso_byte_reg.valid)
    val shifter_bits_source  = Mux(
      miso_data_source_valid,
      miso_data_source_bits,
      next_miso_byte_reg.bits
    )

    val miso_buffer_reg_valid_next =
      Mux(load_shifter, shifter_valid_source, miso_buffer_reg.valid)
    val miso_buffer_reg_bits_next  = Mux(
      load_shifter,
      shifter_bits_source,
      Cat(miso_buffer_reg.bits(6, 0), 0.U)
    )
    ...
    val next_miso_byte_reg_valid_next = Mux(
      miso_data_source_valid,
      true.B,
      Mux(stage_is_consumed, false.B, next_miso_byte_reg.valid)
    )
    val next_miso_byte_reg_bits_next  = Mux(
      miso_data_source_valid,
      miso_data_source_bits,
      next_miso_byte_reg.bits
    )
  ...
  val sram_addr_next = Mux(
    write_cmd_fire,
    0.U,
    Mux(sram_addr_inc, sram_addr_reg + 1.U, sram_addr_reg)
  )

  tl_d_q.io.deq.ready := MuxCase(
    false.B,
    Seq(
  ...
  val tl_beat_count_next = Mux(
    tl_read_state_reg === TlReadState.sWaitBeatAck &&
      tl_d_q.io.deq.fire &&
  ...
  val tl_write_state_next = MuxCase(
    tl_write_state_reg,
    Seq(
  ...
  val tl_write_beat_count_next = Mux(
    tl_write_state_reg === TlWriteState.sWaitBeatAck &&
      tl_d_q.io.deq.fire &&
      !tl_d_q.io.deq.bits.error,
    tl_write_beat_count_reg + 1.U,
    tl_write_beat_count_reg
  )
  val is_last_beat_ack         =
    (tl_write_state_reg === TlWriteState.sWaitBeatAck) && tl_d_q.io.deq.fire && (tl_write_beat_count_reg === tl_write_len_fsm_reg)
...
object Spi2TLUL_128_Emitter extends App {
  ...
  (new ChiselStage).execute(
    ...
    Seq(ChiselGeneratorAnnotation(() => new Spi2TLUL(p))) ++ Seq(
