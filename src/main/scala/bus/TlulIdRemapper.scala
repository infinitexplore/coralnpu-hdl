...
class TlulIdRemapper[A_USER <: Data, D_USER <: Data](
  ...
  val io = IO(new Bundle {
    val tl_a_in  =
      Flipped(Decoupled(new TileLink_A_ChannelBase(p_tl_in, userAGen)))
    val tl_a_out = Decoupled(new TileLink_A_ChannelBase(p_tl_out, userAGen))
    val tl_d_in  =
      Flipped(Decoupled(new TileLink_D_ChannelBase(p_tl_out, userDGen)))
  ...
  val saved_source_id_map = RegInit(
    VecInit(Seq.fill(numTrackerSlots)(0.U(p_tl_in.o.W)))
  )
