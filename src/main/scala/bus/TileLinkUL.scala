...
class TileLink_A_ChannelBase[T <: Data](p: TLULParameters, val userGen: () => T)
    extends Bundle {
  val opcode  = UInt(3.W)
  val param   = UInt(3.W)
  val size    = UInt(p.z.W)
...
class TileLink_D_ChannelBase[T <: Data](p: TLULParameters, val userGen: () => T)
    extends Bundle {
  val opcode = UInt(3.W)
  val param  = UInt(3.W)
  val size   = UInt(p.z.W)
...
class TileLink_A_Channel(p: TLULParameters)
    extends TileLink_A_ChannelBase(p, () => new NoUser) {}
class TileLink_D_Channel(p: TLULParameters)
    extends TileLink_D_ChannelBase(p, () => new NoUser) {}
