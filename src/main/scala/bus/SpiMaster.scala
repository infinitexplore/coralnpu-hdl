 ...
 *
 *  This module manages the SPI protocol state machine, baud rate generation,
 *  and data buffering using internal FIFOs. It interfaces with the bus via a
 *  TileLink-UL slave port.
 */
class SpiMasterCtrl(p: Parameters) extends Module {
  ...
  when(tl_a.fire) {
    ...
    val is_known_addr =
      VecInit(STATUS, CONTROL, TXDATA, RXDATA, CSID, CSMODE).contains(
        addr_offset
      )
  ...
  // Generate response integrity bits
 ...
 *
 *  Wraps the synchronous controller and uses TlulFifoAsync to bridge between
 *  high-speed system bus (clk_i) and potentially slower SPI clock (spi_clk_i).
 */
class SpiMaster(p: Parameters) extends RawModule {
  ...
  val spi_reset = withClock(io.spi_clk_i) {
    RegNext(RegNext(!io.rst_ni.asBool))
  }
