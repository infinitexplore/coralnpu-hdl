...
/**
 *  TLUL2Axi: A Chisel module that serves as a bridge between a TileLink-UL
 *  master and an AXI4 slave.
 *
 *  This module translates TileLink Get and Put operations into AXI read and
 *  write transactions, respectively. It uses a dataflow approach with queues
 *  and an arbiter to manage the protocol conversion.
 *
 *  @param p
 *    The CoralNPU parameters.
 */
class TLUL2Axi[A_USER <: Data, D_USER <: Data](
    p_tl: Parameters,
    p_axi: Parameters,
    userAGen: () => A_USER,
    userDGen: () => D_USER
  ...
  val io     = IO(new Bundle {
    val tl_a = Flipped(
      Decoupled(new TileLink_A_ChannelBase(tlul_p, userAGen))
    ) // TileLink Input
    val tl_d =
      Decoupled(new TileLink_D_ChannelBase(tlul_p, userDGen)) // TileLink Output
    val axi =
      new AxiMasterIO(p_axi.axi2AddrBits, p_axi.axi2DataBits, p_axi.axi2IdBits)
  })
  ...
  val tl_a_q_remapped = Wire(
    Decoupled(new TileLink_A_ChannelBase(p_tl_remapped, userAGen))
  )

  val d_channel_arb = Module(
    new CoralNPURRArbiter(
      new TileLink_D_ChannelBase(p_tl_remapped, userDGen),
      2
    )
  )
  ...
  if (idWidthMismatch) {
    id_remapper = Module(
      new TlulIdRemapper(tlul_p, p_tl_remapped, userAGen, userDGen)
    )
  ...
  val is_put =
    tl_a_q_remapped.bits.opcode === TLULOpcodesA.PutFullData.asUInt ||
  ...
  val aw_q = Module(
    new Queue(
      new AxiAddress(p_axi.axi2AddrBits, p_axi.axi2DataBits, p_axi.axi2IdBits),
      1
    )
  )
  ...
  val w_q = Module(
    new Queue(new AxiWriteData(p_axi.axi2DataBits, p_axi.axi2IdBits), 1)
  )
  ...
  val put_ready =
    aw_q.io.enq.ready && w_q.io.enq.ready && write_tx_info_q.io.enq.ready
  ...
  read_tx_info_q.io.enq.bits.size := Mux(
    (p_axi.axi2DataBits == 256).B,
    5.U,
    tl_a_q_remapped.bits.size
  )

  write_tx_info_q.io.enq.valid     := tl_a_q_remapped.valid && is_put && aw_q.io.enq.ready && w_q.io.enq.ready
  write_tx_info_q.io.enq.bits.size := Mux(
    (p_axi.axi2DataBits == 256).B,
    5.U,
    tl_a_q_remapped.bits.size
  )
  ...
  val read_response  = Wire(
    Decoupled(new TileLink_D_ChannelBase(p_tl_remapped, userDGen))
  )
  val write_response = Wire(
    Decoupled(new TileLink_D_ChannelBase(p_tl_remapped, userDGen))
  )
  ...
  read_tx_info_q.io.deq.ready := io.axi.read.data.valid && d_channel_arb.io
    .in(0)
    .ready

  io.axi.write.resp.ready      := d_channel_arb.io.in(1).ready
  write_tx_info_q.io.deq.ready := d_channel_arb.io
    .in(1)
    .ready && io.axi.write.resp.valid
