...
/**
 *  Axi2TLUL: A Chisel module that serves as a bridge between an AXI4 master and
 *  a TileLink-UL slave.
 *
 *  This module translates AXI read and write transactions into TileLink Get and
 *  Put operations, respectively. It uses a dataflow approach with queues to
 *  manage the protocol conversion.
 *
 ...
 */
class Axi2TLUL[A_USER <: Data, D_USER <: Data](
    p: Parameters,
    userAGen: () => A_USER,
    userDGen: () => D_USER
) extends Module {
  ...
  val io     = IO(new Bundle {
    val axi  =
      Flipped(new AxiMasterIO(p.axi2AddrBits, p.axi2DataBits, p.axi2IdBits))
    val tl_a =
      Decoupled(new TileLink_A_ChannelBase(tlul_p, userAGen)) // TileLink Output
    val tl_d = Flipped(
      Decoupled(new TileLink_D_ChannelBase(tlul_p, userDGen))
    ) // TileLink Input
  })

  assert(
    io.axi.read.addr.bits.len === 0.U || !io.axi.read.addr.valid,
    "Axi2TLUL: AXI read bursts not supported"
  )
  assert(
    io.axi.write.addr.bits.len === 0.U || !io.axi.write.addr.valid,
    "Axi2TLUL: AXI write bursts not supported"
  )
  ...
  private def axiToTl(
      addr: AxiAddress,
      data: Option[AxiWriteData]
  ): TileLink_A_ChannelBase[A_USER] = {
    ...
    tl_a.opcode  := data
      ...
      .getOrElse(TLULOpcodesA.Get.asUInt)
    tl_a.param   := 0.U
  ...
  val arb = Module(
    new CoralNPURRArbiter(new TileLink_A_ChannelBase(tlul_p, userAGen), 2)
  )
  ...
  (new ChiselStage).execute(
    ...
    Seq(
      ChiselGeneratorAnnotation(() =>
        new Axi2TLUL(p, () => new NoUser, () => new NoUser)
      )
