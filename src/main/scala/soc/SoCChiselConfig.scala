...
import coralnpu.{MemoryRegion, MemoryRegions, MemorySize, Parameters}
 ...
 *  @param modulePort
 *    The full path to the port on the instantiated module (e.g., "io.halted",
 *    "io.spi.csb").
 */
...
/**
 *  Defines the parameters for a Chisel module to be instantiated within the
 *  subsystem.
 *
 *  @param name
 ...
 *    A map where keys are port names on the module that are TileLink hosts, and
 *    values are the names of the host ports on the crossbar to connect to.
 *  @param deviceConnections
 *    A map where keys are port names on the module that are TileLink devices,
 *    and values are the names of the device ports on the crossbar to connect
 *    to.
 *  @param externalPorts
 *    A sequence of non-TileLink ports that need to be wired to the subsystem's
 *    top-level IO.
 */
