...
class CoralNPUChiselSubsystemIO(
  ...
  val asyncDeviceDomains  =
    cfg.devices.map(_.clockDomain).distinct.filter(_ != "main")
  ...
  val internalHosts   = SoCChiselConfig(itcmSize, dtcmSize).modules
    .flatMap(_.hostConnections.values)
    .toSet
  val internalDevices = SoCChiselConfig(itcmSize, dtcmSize).modules
    .flatMap(_.deviceConnections.values)
    .toSet
  ...
  val externalHostPorts   =
    cfg.hosts(enableTestHarness).filterNot(h => internalHosts.contains(h.name))
  ...
  val external_hosts = Flipped(new Bundle {
    val ports = MixedVec(externalHostPorts.map { h =>
      new OpenTitanTileLink.Host2Device(
        hostParams(cfg.hosts(enableTestHarness).indexWhere(_.name == h.name))
      )
    })
  })
  ...
  val external_devices = new Bundle {
    val ports = MixedVec(externalDevicePorts.map { d =>
      new OpenTitanTileLink.Host2Device(
        deviceParams(cfg.devices.indexWhere(_.name == d.name))
      )
    })
  }
  ...
  val allExternalPortsConfig =
    SoCChiselConfig(itcmSize, dtcmSize).modules.flatMap(_.externalPorts)
...
class CoralNPUChiselSubsystem(
  ...
  withClockAndReset(io.clk_i, (!io.rst_ni.asBool).asAsyncReset) {
    ...
    def instantiateModule(config: ChiselModuleConfig): BaseModule = {
      config.params match {
        case p: CoreTlulParameters =>
          ...
          core_p.enableDebug = p.enableDebug
    ...
    val externalPortsMap =
      io.allExternalPortsConfig.map(_.name).zip(io.external_ports).toMap
    ...
    instantiatedModules.foreach { case (name, module) =>
      ...
      modulePorts
        .get(s"$name.io.reset")
        .foreach(_ := (!io.rst_ni.asBool).asAsyncReset)
    }

    // Connect all modules based on the configuration.
    SoCChiselConfig(itcmSize, dtcmSize).modules.foreach { config =>
      config.hostConnections.foreach { case (modulePort, xbarPort) =>
        modulePorts(s"${config.name}.$modulePort") <> xbar.io.hosts(
          hostMap(xbarPort)
        )
      }
      config.deviceConnections.foreach { case (modulePort, xbarPort) =>
        xbar.io.devices(deviceMap(xbarPort)) <> modulePorts(
          s"${config.name}.$modulePort"
        )
      }
      config.externalPorts.foreach { extPort =>
        ...
        if (extPort.direction == In)
          moduleIo := topIo.asTypeOf(chiselTypeOf(moduleIo))
        else topIo := moduleIo.asTypeOf(chiselTypeOf(topIo))
      }
    }

    // Connect external-facing TileLink ports
    io.externalHostPorts.map(_.name).zip(io.external_hosts.ports).foreach {
      case (name, port) =>
        xbar.io.hosts(hostMap(name)) <> port
    }
    io.externalDevicePorts.map(_.name).zip(io.external_devices.ports).foreach {
      case (name, port) =>
    ...
    asyncHostDomainMap.foreach { case (domainName, index) =>
    ...
    asyncDeviceDomainMap.foreach { case (domainName, index) =>
    ...
    val ddr_mem_bridge =
      Module(new TlulWidthBridge(xbar.commonParams, ddr_mem_256_tlul_p))

    // Instantiate the AXI converter: 256-bit TL to 256-bit AXI.
    val ddr_mem_axi_conv = Module(
      new TLUL2Axi(
        ddr_mem_256_coralnpu_p,
        ddr_mem_axi_p,
        () => new OpenTitanTileLink_A_User,
        () => new OpenTitanTileLink_D_User
      )
...
object CoralNPUChiselSubsystemEmitter extends App {
  ...
  var itcmSizeKBytes =
    Parameters.itcmSizeKBytesDefault // Default ITCM size in KBytes
  var dtcmSizeKBytes =
    Parameters.dtcmSizeKBytesDefault // Default DTCM size in KBytes
  ...
  val hostParams   =
    SoCChiselConfig(itcmSize, dtcmSize).crossbar.hosts(enableTestHarness).map {
      host =>
        val p = new Parameters
  ...
  args.foreach {
    case s if s.startsWith("--target-dir=") =>
      targetDir = Some(s.stripPrefix("--target-dir="))
  ...
  lazy val subsystem = new CoralNPUChiselSubsystem(
    hostParams,
    deviceParams,
    enableTestHarness,
    itcmSize,
