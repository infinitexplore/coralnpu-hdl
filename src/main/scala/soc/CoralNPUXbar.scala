 ...
 *
 *  This bundle's ports are derived from the CrossbarConfig object. It
 *  automatically creates clock and reset ports for any asynchronous domains
...
class CoralNPUXbarIO(
  ...
  // --- Host (Master) Ports ---
  val hosts = Flipped(
    MixedVec(hostParams.map(p => new OpenTitanTileLink.Host2Device(p)))
  )

  // --- Device (Slave) Ports ---
  val devices = MixedVec(
    deviceParams.map(p => new OpenTitanTileLink.Host2Device(p))
  )
  ...
  val async_ports_devices = Input(
    Vec(
      asyncDeviceDomains.length,
      new Bundle {
        val clock = Clock()
  ...
  val async_ports_hosts = Input(
    Vec(
      asyncHostDomains.length,
      new Bundle {
        val clock = Clock()
        val reset = AsyncReset()
      }
 ...
 *
 *  This RawModule constructs a crossbar by interpreting the CrossbarConfig
 *  object. This gives explicit control over clock and reset signals, which is
 *  critical for a multi-domain design.
 *
 *  @param p
...
class CoralNPUXbar(
  ...
  override val desiredName =
    if (enableTestHarness) "CoralNPUXbarTestHarness" else "CoralNPUXbar"
  ...
  val (hostInterfaces, deviceInterfaces, hostSockets, deviceSockets) =
    withClockAndReset(clock, reset) {
      ...
      val hostInterfaces = cfg
        ...
        .map { host =>
          ...
          if ((hostParams(hostId).w * 8) < commonWidth) {
            val bridge =
              Module(new TlulWidthBridge(hostParams(hostId), commonParams))
      ...
      val deviceInterfaces = cfg.devices.map { device =>
        ...
        val standardizedIface                               =
          Wire(new bus.OpenTitanTileLink.Host2Device(commonParams))
        ...
        if ((deviceParams(deviceId).w * 8) != commonWidth) {
          val bridge = if (device.clockDomain != "main") {
            val domainIndex = asyncDeviceDomainMap(device.clockDomain)
            withClockAndReset(
              io.async_ports_devices(domainIndex).clock,
              io.async_ports_devices(domainIndex).reset
            ) {
