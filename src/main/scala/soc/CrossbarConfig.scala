    ...
    if (itcmSize == defaultItcmSize && dtcmSize == defaultDtcmSize) {
      ...
      Seq(
        ...
        AddressRange(
          0x00030000,
          0x00001000
        )                                     // 4kB peripheral space (default peripheral address)
      )
    } else {
      ...
      Seq(
        ...
        AddressRange(
          0x00200000,
          0x00001000
        ) // 4kB peripheral space (custom peripheral address)
      )
    }
  }
 ...
 *
 *  This object can be run to check for configuration errors, such as
 *  overlapping address ranges between devices.
 */
object CrossbarConfigValidator extends App {
          ...
          if (overlap) {
            val errorMsg =
              s"""
                 ...
                 |  Device 1: ${dev1.name} -> Range [0x${start1.toString(
                  16
                )}, 0x${(end1 - 1).toString(16)}]
                 |  Device 2: ${dev2.name} -> Range [0x${start2.toString(
                  16
                )}, 0x${(end2 - 1).toString(16)}]
               """
  ...
  def printConfig(enableTestHarness: Boolean): Unit = {
    ...
    CrossbarConfig().devices.foreach { d =>
      ...
      d.addr.foreach { a =>
        println(
          f"    - 0x${a.base}%08x - 0x${a.base + a.size - 1}%08x (Size: ${a.size / 1024}kB)"
        )
      }
    }
