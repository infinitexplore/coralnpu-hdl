...
class FifoIxO[T <: Data](t: T, i: Int, o: Int, n: Int /* depth */ )
    extends Module {
  ...
  val icount =
    (io.in.bits.map(x => x.valid.asUInt).reduce(_ +& _))(log2Ceil(i), 0)

  val oactiveBits = Cat(
    (0 until o).reverse.map(x => io.out(x).valid && io.out(x).ready)
  )

  val ovalid = oactiveBits =/= 0.U

  val ocount =
    (0 until o).map(x => oactiveBits(x).asUInt).reduce(_ +& _)(log2Ceil(o), 0)

  for (n <- 1 until o) {
    assert(
      !(oactiveBits(n) === 1.U && oactiveBits(n - 1, 0) =/= ((1 << n) - 1).U)
    )
  }
  ...
  for (n <- 1 until o) {
    assert(
      !(ovalidBits(n) === 1.U && ovalidBits(n - 1, 0) =/= ((1 << n) - 1).U)
    )
  }
  ...
  for (n <- 1 until o) {
    assert(
      !(oreadyBits(n) === 1.U && oreadyBits(n - 1, 0) =/= ((1 << n) - 1).U)
    )
  }
  ...
  for (q <- 0 until m) {
    ...
    if (true) {
      val data = (0 until i)
        .map(x => MuxOR(valid(x), io.in.bits(x).bits.asUInt))
        .reduce(_ | _)
