...
object DecoupledVectorIO {
  def apply[T <: Data](gen: T, n: Int): DecoupledVectorIO[T] =
    new DecoupledVectorIO(gen, n)
}
...
class InstructionBuffer[T <: Data](val gen: T, val n: Int, val window: Int)
    extends Module {
  assert(window % n == 0)
  ...
  val circularBuffer = Module(
    new CircularBufferMulti(t = gen, n = n, capacity = window)
  )

  // Enqueue Logic
  val feedInReady =
    Mux(circularBuffer.io.nSpace < n.U, circularBuffer.io.nSpace, n.U)
  ...
  for (nIndex <- 0 until n) {
    io.out(nIndex)
      .valid            := (nIndex.U < circularBuffer.io.nEnqueued) && !io.flush
