...
object GenerateCoreShimSource {
  def apply(p: Parameters): String = {
    ...
    coreInstantiation += """  fpnew_top#(
      ...
      .replaceAll(
        "DIVSQRT_SEL",
        if (p.floatPulpDivsqrt != 0) "fpnew_pkg::PULP" else "fpnew_pkg::TH32"
      )
      .stripMargin
...
class FloatCoreWrapper(p: Parameters)
  ...
  val io = IO(new Bundle {
    ...
    val operands_i =
      Input(Vec(FpNewConfig.NUM_OPERANDS, UInt(FpNewConfig.WIDTH.W)))
  ...
  if (p.floatPulpDivsqrt == 0) {
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/clk/rtl/gated_clk_cell.v"
    )
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/fdsu/rtl/pa_fdsu_ctrl.v"
    )
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/fdsu/rtl/pa_fdsu_ff1.v"
    )
    addResource(
    ...
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/fdsu/rtl/pa_fdsu_top.v"
    )
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/fpu/rtl/pa_fpu_dp.v"
    )
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/fpu/rtl/pa_fpu_frbus.v"
    )
    addResource(
      "external/cvfpu/vendor/opene906/E906_RTL_FACTORY/gen_rtl/fpu/rtl/pa_fpu_src_type.v"
    )
...
class FloatCore(p: Parameters) extends Module {
  ...
  instQueue.io.enq <> MakeDecoupled(
    io.inst.valid,
    instQueue.io.count === 0.U,
    io.inst.bits
  )
  ...
  val read_port_1_valid =
    op_i.isOneOf(FpNewOperation.FMADD, FpNewOperation.FNMSUB) ||
      (
  ...
  val read_port_2_valid = op_i.isOneOf(
    FpNewOperation.FMADD,
    FpNewOperation.FNMSUB
  ) ||
  ...
  val rnd_mode                 = MuxCase(
    ...
    Seq(
      ...
      (csr_rm_valid && (csr_rm === FpNewRoundingMode.DYN)) -> MakeValid(
        false.B,
        csr_rm
      ),
  ...
  io.write_ports(0).data := Fp32.fromWord(
    Mux(fmv_w_x, io.rs1.data, floatCoreWrapper.io.result_o)
  )
