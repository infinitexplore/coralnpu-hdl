...
class RvvCoreWrapper(p: Parameters)
  ...
  val io = IO(new Bundle {
    ...
    val inst =
      Vec(p.instructionLanes, Flipped(Decoupled(new RvvCompressedInstruction)))
  ...
  setInline(
    "RvvCoreWrapper.sv",
    GenerateCoreShimSource(p.instructionLanes, p.rvvVlen)
  )
}
