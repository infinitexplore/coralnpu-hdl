...
object RvvCompressedInstruction {
  def from_uncompressed(
      inst: UInt,
      pc: UInt
  ): Valid[RvvCompressedInstruction] = {
  ...
  private def s1decode_opivv(
    ...
    val op         = MuxUpTo1H(
      ...
      Seq(
        ...
        (f6vm === BitPat("b001100_?")) -> MakeValid(
          no_overlap,
          RvvAluOp.VRGATHER
        ),
        (f6vm === BitPat("b001110_?")) -> MakeValid(
          no_overlap,
          RvvAluOp.VRGATHEREI16
        ),
        // No VSLIDEUP.
        // No VSLIDEDOWN.
        (f6vm === "b010000_0".U)       -> MakeValid(
          vd =/= "b00000".U,
          RvvAluOp.VADC
        ), // Mask required.
        (f6vm === BitPat("b010001_?")) -> MakeValid(RvvAluOp.VMADC),
        (f6vm === "b010010_0".U)       -> MakeValid(
          vd =/= "b00000".U,
          RvvAluOp.VSBC
        ), // Mask required.
        (f6vm === BitPat("b010011_?")) -> MakeValid(RvvAluOp.VMSBC),
        (f6vm === "b010111_0".U)       -> MakeValid(
          RvvAluOp.VMERGE
        ), // Mask required.
  ...
  private def s1decode_opivx(
    ...
    val op         = MuxUpTo1H(
      ...
      Seq(
        ...
        (f6vm === BitPat("b001111_?")) -> MakeValid(
          no_overlap,
          RvvAluOp.VSLIDEDOWN
        ),
        (f6vm === "b010000_0".U)       -> MakeValid(
...
object RvvS1DecodeCompressedInstruction extends RvvS1DecodeInstructionBase {
  ...
  def apply(
      inst: Valid[RvvCompressedInstruction]
  ): Valid[RvvS1DecodedInstruction] = {
