            ...
            Mux(
              ...
              Mux(
                ...
                Mux(
                  psel(5),
                  7.U,
                  Mux(psel(6), 8.U, Mux(psel(7), 9.U, Mux(psel(8), 10.U, 0.U)))
                )
              )
            )
          )
...
object WithAddr {
  def apply[T <: Data](width: Int, gen: T): WithAddr[T] =
    new WithAddr(width, gen)
...
/**
 *  A transformation that lifts a function f = (x => y) to a new function f =
 *  (WithAddr[x] => WithAddr[y]). The addr field of the lifted function is
 *  preserved between the inputs and outputs.
 *  @param width
 *    The bit-width of the addr field.
 *  @param f
...
/**
 *  Generates two bitmasks based on an address, transaction sizes, and the total
 *  number of bytes. These masks are typically used to identify which bytes are
 *  affected by two sequential transactions.
 *
 *  @param nBytes
 *    The total number of bytes for which the mask is being generated.
 *  @param addr
 *    The starting address of the first transaction. The lower bits of this
 *    address are used as an offset.
 *  @param txnSizes
...
object GenerateMasks {
  def apply(nBytes: Int, addr: UInt, txnSizes: Vec[UInt]): (UInt, UInt) = {
    ...
    val mask0  = VecInit((0 until nBytes).map(i => i.U < txnSizes(0))).asUInt
      .rotateLeft(bottom)
    val mask1  = VecInit((0 until nBytes).map(i => i.U < txnSizes(1))).asUInt
      .rotateLeft(bottom + txnSizes(0))
    (mask0, mask1)
  }
}
