...
class SRAM(p: Parameters, sramAddressWidth: Int) extends Module {
  ...
  io.sram.address := MuxUpTo1H(
    ...
    Seq(
      ...
      io.fabric.readDataAddr.valid  -> io.fabric.readDataAddr
        .bits(sramAddressWidth + lsb - 1, lsb)
    )
  )
  ...
  io.sram.writeData := Mux(
    io.fabric.writeDataAddr.valid,
    writeDataVec,
    0.U.asTypeOf(writeDataVec)
  )
  val readMaskData = RegInit(
    VecInit(Seq.fill(io.fabric.writeDataBits.getWidth / 8)(true.B))
  )
