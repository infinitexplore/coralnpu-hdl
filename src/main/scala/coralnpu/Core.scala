...
class Core(p: Parameters, moduleName: String)
    extends Module
...
object EmitCore extends App {
  ...
  val systemVerilogSource   =
    ChiselStage.emitSystemVerilog(core, chiselArgs.toArray, firtoolOpts)
    ...
    case Some(targetDir) => {
      {
        ...
        val zip      = new ZipOutputStream(
          new FileOutputStream(targetDir + "/" + coreName + ".zip")
        )
