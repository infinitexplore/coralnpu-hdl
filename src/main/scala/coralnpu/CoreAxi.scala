...
class CoreAxi(p: Parameters, coreModuleName: String) extends RawModule {
  ...
  val io                   = IO(new Bundle {
    ...
    val axi_master =
      new AxiMasterIO(p.axi2AddrBits, p.axi2DataBits, p.axi2IdBits)
  ...
  val global_reset =
    (!Mux(io.te, io.aresetn, rst_sync.io.rstn_o).asBool).asAsyncReset
  withClockAndReset(rst_sync.io.clk_o, global_reset) {
    ...
    if (p.useDebugModule) {
      ...
      dmEnable := true.B
      val dmReqArbiter =
        Module(new CoralNPURRArbiter(new DebugModuleReqIO(p), 2))
      ...
      dm.get.io.ext.rsp.ready := inflight.io.deq.valid && Mux(
        rspId === 1.U,
        csr.io.debug.get.rsp.ready,
        io.dm.get.rsp.ready
      )
    }
    ...
    val core_reset = Mux(
      ...
      (csr.io.reset || dm.map(_.io.ndmreset).getOrElse(false.B)).asAsyncReset
    )
    val core       = withClockAndReset(cg.io.clk_o, core_reset) {
      Core(p, coreModuleName)
    }
    cg.io.enable            := io.irq || (!csr.io.cg && !core.io.wfi) || dm
      .map(_.io.haltreq(0))
      .getOrElse(false.B)
    ...
    if (p.useDebugModule) {
      ...
      dm.get.io.resumeack(0)    := !core.io.dm.get.debug_mode && RegNext(
        core.io.dm.get.debug_mode,
        false.B
      )
    ...
    axiSlave.io.axi.write.addr <> GateDecoupled(
      io.axi_slave.write.addr,
      axiSlaveEnable
    )
    axiSlave.io.axi.write.data <> GateDecoupled(
      io.axi_slave.write.data,
      axiSlaveEnable
    )
    io.axi_slave.write.resp <> GateDecoupled(
      axiSlave.io.axi.write.resp,
      axiSlaveEnable
    )
    axiSlave.io.axi.read.addr <> GateDecoupled(
      io.axi_slave.read.addr,
      axiSlaveEnable
    )
    io.axi_slave.read.data <> GateDecoupled(
      axiSlave.io.axi.read.data,
      axiSlaveEnable
    )
