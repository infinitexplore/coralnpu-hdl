...
class L1ICache(p: Parameters) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val axi      = new Bundle {
      val read =
        new AxiMasterReadIO(p.axi0AddrBits, p.axi0DataBits, p.axi0IdBits)
    }
  ...
  for (i <- 0 until slots) {
    val set      = i / assoc
    val setMatch =
      if (assoc == slots) true.B else io.ibus.addr(setMsb, setLsb) === set.U
    matchSet(i) := setMatch
  }
  ...
  for (i <- 0 until slots / assoc) {
    ...
    when(
      io.ibus.valid && io.ibus.ready && (if (assoc == slots) true.B
                                           ...
                                           io.ibus.addr(setMsb, setLsb) === i.U)
    ) {
      for (j <- 0 until assoc) {
        when(matchSet(j)) {
