...
class DecodedInstruction(p: Parameters) extends Bundle {
  ...
  def isAlu1Bit(): Bool     = {
    clz || ctz || cpop || sextb || sexth || zexth || orcb || rev8
  }
  ...
  def isFloatLoad(): Bool   = {
    float
      .map(f => f.valid && f.bits.opcode === FloatOpcode.LOADFP)
      .getOrElse(false.B)
  }
  def isFloatStore(): Bool  = {
    float
      .map(f => f.valid && f.bits.opcode === FloatOpcode.STOREFP)
      .getOrElse(false.B)
  }
  def isLsu(): Bool         = {
    isScalarLoad() || isScalarStore() || flushat || flushall ||
    isFloatLoad() || isFloatStore() || (if (p.enableRvv) {
                                          rvv.get.valid && rvv.get.bits
                                            .isLoadStore()
                                        } else {
                                          false.B
                                        })
  ...
  def isFency(): Bool       = {
    fencei || ebreak || wfi || mpause || flushat || flushall
  }
  ...
  def floatWritesRd(): Bool       = {
    float.map(f => f.valid && f.bits.scalar_rd).getOrElse(false.B)
  }
  ...
  def floatReadsRs2(): Bool       = {
    float.map(f => f.valid && f.bits.uses_rs2).getOrElse(false.B)
  }
  def floatReadsRs3(): Bool       = {
    float.map(f => f.valid && f.bits.uses_rs3).getOrElse(false.B)
  }
...
class Dispatch(p: Parameters) extends Module {
  val io = IO(new Bundle {
    ...
    val rvvFault   =
      Option.when(p.enableRvv)(Output(Vec(p.instructionLanes, Bool())))
    ...
    val inst =
      Vec(p.instructionLanes, Flipped(Decoupled(new FetchInstruction(p))))
    ...
    val rvvRdMark  = Option.when(p.enableRvv)(
      Vec(p.instructionLanes, Flipped(new RegfileWriteAddrIO))
    )
...
class DispatchV2(p: Parameters) extends Dispatch(p) {
  ...
  val configInvalid = if (p.enableRvv) {
    val configChange =
      decodedInsts.map(x => x.rvv.get.valid && x.rvv.get.bits.isVset())
  ...
  val rvvConfigInterlock = if (p.enableRvv) {
    val canDispatchRvv = (0 until p.instructionLanes).map(i =>
      !decodedInsts(i).rvv.get.valid || // Don't lock non-rvv
        !decodedInsts(i).rvv.get.bits
          .isLoadStore() ||             // Non-LSU can handle change
  ...
  val rvvInterlock = if (p.enableRvv) {
    ...
    (0 until p.instructionLanes).map(i =>
      isRvvCount(i) < io.rvvQueueCapacity.get
    )
  } else {
    Seq.fill(p.instructionLanes)(true.B)
  }
  ...
  for (i <- 0 until p.instructionLanes) {
    ...
    val bru_target =
      io.inst(i).bits.addr + Mux(io.inst(i).bits.inst(2), d.immjal, d.immbr)
    ...
    val jalrFault =
      tryDispatch && bru.valid && (bru.bits === BruOp.JALR) && ((io
        .jalrTarget(i)
    ...
    io.bru(i)
      .valid        := tryDispatch && bru.valid && !(jalFault || jalrFault || bxxFault)
    ...
    if (i == 0) {
      ...
      val isVxsatOrVcsr                    =
        csr_bits_index === 0x009.U || csr_bits_index === 0x00f.U
      val rvvIdleOrNotVxsat                = io.rvvIdle.getOrElse(true.B) || !isVxsatOrVcsr
      io.csr.valid      := tryDispatch && csr.valid && csr_address_valid && (if (
                                                                          p.enableFloat
                                                                        ) {
                                                                          io.float.get.ready
                                                                        } else {
    ...
    if (p.enableFloat && (i == 0)) {
      io.float.get.valid := tryDispatch && d.float.get.valid && !(d
        .isFloatLoad() || d.isFloatStore())
    ...
    val dispatched = Seq(
      ...
      Option
        .when(i == 0)(Seq(io.csr.valid, io.slog, fenceValid))
        .getOrElse(Seq()) ++
      Option.when(p.enableRvv)(Seq(io.rvv.get(i).fire)).getOrElse(Seq()) ++
      Option
        .when(p.enableFloat && i == 0)(Seq(io.float.get.fire))
        .getOrElse(Seq())
  ...
  if (p.enableRvv) {
    for (i <- 0 until p.instructionLanes) {
      io.rvvFault.get(i) := (if (i == 0) {
                               ...
                               val requireZeroVstart =
                                 decodedInsts(i).rvv.get.valid &&
                                   decodedInsts(0).rvv.get.bits
                                     .requireZeroVstart()
                               val vStartNotZero     = io.rvvState.get.valid &&
                                 (io.rvvState.get.bits.vstart =/= 0.U)
                               io.inst(0)
                                 .valid && requireZeroVstart && vStartNotZero
                             } else {
                               false.B
                             })
    }
  ...
  for (i <- 0 until p.instructionLanes) {
    ...
    io.rs1Set(i).value := Mux(
      d.isCsr(),
      d.immcsr,
      io.inst(i).bits.addr
    ) // Program Counter (PC)
    io.rs2Set(i).valid := io.inst(i).fire && d.rs2Set()
    io.rs2Set(i).value := MuxCase(
      d.imm12,
    ...
    val rdMark_valid =
      io.alu(i).fire || io.mlu(i).fire || io.dvu(i).fire ||
        ...
        (if (i == 0) {
           io.float.map(x => x.fire && x.bits.scalar_rd).getOrElse(false.B)
         } else { false.B }) ||
        (io.bru(i).valid && (io
          .bru(i)
          .bits
          .op
          .isOneOf(BruOp.JAL, BruOp.JALR)) && rdAddr(i) =/= 0.U)
    ...
    if (p.enableFloat && (i == 0)) {
      val rdMark_flt_valid =
        (io.float.get.fire && !d.float.get.bits.scalar_rd) || (io
          .lsu(i)
          .fire && d.isFloatLoad())
    ...
    if (p.enableRvv) {
      val rvvRdMark_valid =
        io.rvv.get(i).fire && d.rvv.get.bits.writesVectorRegister()
...
object DecodeInstruction {
  def apply(
    ...
    d.flushat  := op === BitPat(
      "b0010?_??_00000_?????_000_00000_11101_11"
    ) && op(19, 15) =/= 0.U
