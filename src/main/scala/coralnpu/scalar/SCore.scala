...
class SCore(p: Parameters) extends Module {
  ...
  val retirement_buffer = Module(
    new RetirementBuffer(p, mini = !p.useRetirementBuffer)
  )
  ...
  if (p.enableRvv) {
    ...
    (0 until p.instructionLanes).foreach(i => {
      rob_io.writeDataVector.get(i).valid               := io.rvvcore.get
        .rd_rob2rt_o(i)
        .w_valid
      rob_io.writeDataVector.get(i).bits.addr           := io.rvvcore.get
        .rd_rob2rt_o(i)
        .w_index
      rob_io.writeDataVector.get(i).bits.data           := io.rvvcore.get
        .rd_rob2rt_o(i)
        .w_data
      rob_io.writeDataVector.get(i).bits.uop_pc         := io.rvvcore.get
        .rd_rob2rt_o(i)
        .uop_pc
  ...
  val bru =
    (0 until p.instructionLanes).map(x => Seq(Bru(p, x == 0))).reduce(_ ++ _)
  val mlu = Mlu(p)
  val dvu = Dvu(p)
  ...
  dispatch.io.halted           := csr.io.halted || csr.io.wfi || csr.io.dm
    .map(_.debug_mode)
    .getOrElse(false.B)
  ...
  if (p.useDebugModule) {
    ...
    csr.io.rs1       := Mux(
      RegNext(dispatch.io.csr.valid, false.B),
      regfile.io.readData(0),
      dmRs1
    )
    ...
    val stepTriggered    =
      (!csr.io.dm.get.debug_mode && csr.io.dm.get.dcsr_step && dispatch.io
        .inst(0)
        .fire)
  ...
  for (i <- 0 until p.instructionLanes) {
    ...
    val rvvCoreRdAddr  =
      MuxOR(rvvCoreRdValid, io.rvvcore.map(_.rd(i).bits.addr).getOrElse(0.U))
    val rvvCoreRdData  =
      MuxOR(rvvCoreRdValid, io.rvvcore.map(_.rd(i).bits.data).getOrElse(0.U))
  ...
  val fRegfile        = Option.when(p.enableFloat)(
    Module(new FRegfile(p, floatReadPorts, floatWritePorts))
  )
  if (p.enableFloat) {
    ...
    fRegfile.get.io.scoreboard_set :=
      MuxOR(
        dispatch.io.rdMark_flt.get.valid,
        UIntToOH(dispatch.io.rdMark_flt.get.addr)
      )
    if (p.useDebugModule) {
      ...
      fRegfile.get.io.read_ports(0).addr  := MuxCase(
        ...
        Seq(
          ...
          floatCore.get.io.read_ports(0).valid -> floatCore.get.io
            .read_ports(0)
            .addr
        )
      )
      ...
      fRegfile.get.io.write_ports(0).addr  := MuxCase(
        ...
        Seq(
          ...
          floatCore.get.io.write_ports(0).valid -> floatCore.get.io
            .write_ports(0)
            .addr
        )
      )
    ...
    (0 until 2).foreach(i => {
      ...
      rob_io.writeDataFloat.get(i).bits.addr := fRegfile.get.io
        .write_ports(i)
        .addr
      rob_io.writeDataFloat.get(i).bits.data := fRegfile.get.io
        .write_ports(i)
        .data
        .asWord
    })
  ...
  fault_manager.io.in.fetchFault := MakeValid(
    fetchFaultValid,
    fetch.io.fault.bits
  )
  ...
  debugEn := Cat(
    fetch.io.inst.lanes.map(x => x.valid && x.ready && !branchTaken)
  )

  for (i <- 0 until p.instructionLanes) {
    debugAddr(i) := Mux(
      debugEn(i),
      fetch.io.inst.lanes(i).bits.addr,
      debugAddr(i)
    )
    debugInst(i) := Mux(
      debugEn(i),
      fetch.io.inst.lanes(i).bits.inst,
      debugInst(i)
    )
  }
  ...
  if (p.enableFloat) {
    ...
    for (i <- 0 until 2) {
      io.debug.float.get.writeData(i).valid     := fRegfile.get.io
        .write_ports(i)
        .valid
      io.debug.float.get.writeData(i).bits.addr := fRegfile.get.io
        .write_ports(i)
        .addr
      io.debug.float.get.writeData(i).bits.data := fRegfile.get.io
        .write_ports(i)
        .data
        .asWord
    }
