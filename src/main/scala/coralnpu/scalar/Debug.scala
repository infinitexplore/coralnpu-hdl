...
class DebugModule(p: Parameters) extends Module {
  ...
  val dmcontrol_wvalid =
    (req.fire && req.bits.isAddrDmcontrol && req.bits.isWrite)
  for (i <- 0 until nHart) {
    haltreq(i)   := MuxCase(
      haltreq(i),
      Seq(
  ...
  val cmdtypeIsAccessRegister =
    (req.bits.cmdtype === AccessRegisterCommand.Cmdtype)
  val cmdtypeIsAccessMemory   = (req.bits.cmdtype === AccessMemoryCommand.Cmdtype)
  val regnoIsCsr              =
    (req.bits.regno >= 0.U(16.W)) && (req.bits.regno < "x1000".U(16.W))
  val regnoIsScalar           =
    (req.bits.regno >= "x1000".U(16.W)) && (req.bits.regno < "x1020".U(16.W))
  val regnoIsFloat            =
    (req.bits.regno >= "x1020".U(16.W) && (req.bits.regno < "x1040".U(16.W)))
  ...
  io.csr.valid := io.halted(
    0
  ) && abstractCmdValid && cmdtypeIsAccessRegister && regnoIsCsr
  ...
  val abstractcs_wvalid =
    (req.fire && req.bits.isAddrAbstractcs && req.bits.isWrite)
  ...
  data0 := MuxCase(
    ...
    Seq(
      ...
      (abstractCmdComplete && cmdtypeIsAccessMemory && io.halted(
        0
      ) && req.valid && !req.bits.write && io.itcm.readData.valid)        -> data0ItcmReadData
        .rotateRight(data1(1, 0) * 8.U),
      (abstractCmdComplete && cmdtypeIsAccessMemory && io.halted(
        0
      ) && req.valid && !req.bits.write && io.dtcm.readData.valid)        -> data0DtcmReadData
        .rotateRight(data1(1, 0) * 8.U),
      !dmactive                                                           -> 0.U(32.W)
    )
  )
  ...
  val writeDataBits  =
    data0.asTypeOf(UInt(p.axi2DataBits.W)) << (byteOffset * 8.U)
