...
/**
 *  A unit which calculates branch targets and handles associated control flow
...
class Bru(p: Parameters, first: Boolean) extends Module {
  val io = IO(new Bundle {
    ...
    val fault_manager =
      Option.when(first)(Input(Valid(Flipped(new FaultManagerOutput))))
  })
  ...
  val mtvec           = if (first) { Cat(io.csr.get.out.mtvec(31, 2), 0.U(2.W)) }
  else { 0.U(32.W) }
  ...
  io.taken.valid    := stateReg.valid && MuxLookup(op, pipeline0Taken)(
    Seq(
  ...
  if (first) {
    ...
    io.csr.get.in.mode.bits  := Mux(
      ((op === BruOp.MRET) && (mode === CsrMode.Machine)),
      CsrMode.Machine,
      CsrMode.User
    )
