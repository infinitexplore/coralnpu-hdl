...
class Fetcher(p: Parameters) extends Module {
  ...
  assert(
    (p.fetchDataBits == 128 && lsb == 4) || (p.fetchDataBits == 256 && lsb == 5)
  )
  ...
  val ibusCmd   = RegNext(
    ForceZero(MakeValid(ibusFired, io.ctrl.bits)),
  ...
  for (i <- 0 until p.fetchInstrSlots) {
    ...
    io.fetch.bits
      .inst(i) := io.ibus.rdata(offset + p.instructionBits - 1, offset)
  }
}
...
class FetchControl(p: Parameters) extends Module {
  val io = IO(new Bundle {
    ...
    val bufferRequest =
      DecoupledVectorIO(new FetchInstruction(p), p.fetchInstrSlots)
  ...
  def Predecode(fetchResponse: FetchResponse): PredecodeOutput = {
    ...
    val branchTargets =
      VecInit.tabulate(p.fetchInstrSlots)(i => PredictJump(addrs(i), insts(i)))
    ...
    val hasJumpedBefore = VecInit(
      jumped.scan(false.B)(_ || _).take(p.fetchInstrSlots)
    )
    ...
    val nextFetchPc = MuxUpTo1H(
      ...
      (0 until p.fetchInstrSlots).map(i =>
        firstJumpOH(i) -> branchTargets(i).bits
      )
    )
  ...
  val fetchFaultValid =
    (faulted || (io.fetchData.valid && io.fetchData.bits.fault)) &&
      !io.branch.valid
  ...
  val writeToBuffer =
    io.fetchData.valid && !fetchFaultValid && !ongoingBranchOrFlush
...
class UncachedFetch(p: Parameters) extends FetchUnit(p) {
  ...
  val debug_iflush = if (p.useDebugModule) {
    Seq(
  ...
  instructionBuffer.io.flush := io.iflush.valid || branch.valid || io.debug_pc
    .map(_.valid)
    .getOrElse(false.B)
  ...
  pc       := Mux(
    instructionBuffer.io.out(0).valid,
    instructionBuffer.io.out(0).bits.addr,
    pc
  )
  io.pc    := pc
