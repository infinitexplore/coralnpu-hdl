...
class FaultManager(p: Parameters) extends Module {
  val io = IO(new Bundle {
    val in  = new Bundle {
      ...
      val rvv_fault    =
        Option.when(p.enableRvv)(Input(Valid(new FaultManagerOutput)))
      val undef        = Input(
        Vec(
          p.instructionLanes,
          new Bundle {
  ...
  val rvv_dispatch_fault     =
    io.in.fault.map(_.rvv.getOrElse(false.B)).reduce(_ | _)
  val rvv_dispatch_fault_idx = PriorityEncoder(
    io.in.fault.map(_.rvv.getOrElse(false.B))
  )
  ...
  val first_fault_is_rvv_dispatch =
    (rvv_dispatch_fault && (rvv_dispatch_fault_idx === first_fault))
