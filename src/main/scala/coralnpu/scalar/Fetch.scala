...
class Fetch(p: Parameters) extends FetchUnit(p) {
  ...
  val preBranch        =
    (0 until p.instructionLanes).map(x => Predecode(instAddr(x), instBits(x)))
  ...
  val preBranchTarget = MuxCase(
    ...
    (0 until p.instructionLanes - 1).map(i =>
      preBranchTakens(i) -> preBranchTargets(i)
    )
  )
  ...
  val l0valids =
    (0 until p.instructionLanes).map(x => l0valid(branchIndices(x)))
  val l0validP = l0valid(preBranchIndex)

  val l0tags =
    (0 until p.instructionLanes).map(x => VecAt(l0tag, branchIndices(x)))
  ...
  val reqs      =
    (0 until p.instructionLanes).map(x => reqBValid(x) && !prevValid(x))

  val reqP = preBranchTaken && !l0req(
    preBranchIndex
  ) && (preBranchTag =/= l0tagP || !l0validP)
  val req0 = !match0 && !l0req(instIndex0)
  val req1 = !match1 && !l0req(instIndex1)

  aslice.io.in.valid := (reqs ++ Seq(reqP, req0, req1)).reduce(
    _ || _
  ) && !io.iflush.valid
  aslice.io.in.bits  := MuxCase(
    instAligned1,
    (0 until p.instructionLanes).map(x =>
      reqs(x) -> Cat(io.branch(x).value(31, indexLsb), 0.U(indexLsb.W))
    ) ++
      Array(
  ...
  val nxtInstAddrOffset =
    instAddr.map(x => x) ++ instAddr.map(x => x + (p.instructionLanes * 4).U)
  ...
  def BranchMatchDe(
    ...
    val muxbits1 =
      VecAt(l0data, addr(p.instructionLanes - 1)(indexMsb, indexLsb))
  ...
  val brchDe        =
    (0 until p.instructionLanes).map(x => PredecodeDe(instAddr(x), instBits(x)))
  ...
  val brchTargetDe = MuxCase(
    ...
    (0 until p.instructionLanes - 1).map(x =>
      brchTakensDe(x) -> brchTargetsDe(x)
    )
  )
  ...
  val instLanesReady = Cat(
    (0 until p.instructionLanes).reverse.map(x => io.inst.lanes(x).ready)
  )
  for (i <- 0 until p.instructionLanes - 1) {
    assert(
      !(!instValidUInt(i) && (instValidUInt(
        p.instructionLanes - 1,
        i + 1
      ) =/= 0.U))
    )
    assert(
      !(!instLanesReady(i) && (instLanesReady(
        p.instructionLanes - 1,
        i + 1
      ) =/= 0.U))
    )
