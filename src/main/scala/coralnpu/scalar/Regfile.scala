...
class Regfile(p: Parameters) extends Module {
  ...
  val io              = IO(new Bundle {
    ...
    val writeMask  = Vec(
      p.instructionLanes + extraWritePorts,
      new Bundle { val valid = Input(Bool()) }
    )
  ...
  // ***************************************************************************
  val readDataReady   = RegInit(VecInit(Seq.fill(p.instructionLanes * 2) {
    false.B
  }))
  ...
  val busValid = Cat(
    (0 until p.instructionLanes).reverse.map(x => io.busAddr(x).valid)
  )
  ...
  for (i <- 0 until p.instructionLanes) {
    busAddr(i) := Mux(
      io.busAddr(i).bypass,
      rwdata(2 * i),
      Mux(io.busAddr(i).immen, rdata(2 * i) + io.busAddr(i).immed, rdata(2 * i))
    )
  }
