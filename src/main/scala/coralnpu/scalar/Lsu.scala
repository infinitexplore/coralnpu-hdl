...
class Lsu(p: Parameters) extends Module {
  val io = IO(new Bundle {
    ...
    val busPort_flt =
      Option.when(p.enableFloat)(Flipped(new RegfileBusPortIO(p)))
    ...
    val rvv2lsu =
      Option.when(p.enableRvv)(Vec(2, Flipped(Decoupled(new Rvv2Lsu(p)))))
  ...
  def opSize(op: LsuOp.Type, address: UInt): (UInt, UInt) = {
    ...
    val size = MuxUpTo1H(
      ...
      Seq(
        ...
        op.isOneOf(LsuOp.LH, LsuOp.LHU, LsuOp.SH)   -> Mux(
          halfAligned,
          2.U,
          16.U
        ),
  ...
  def apply(
    ...
    MuxUpTo1H(
      ...
      Seq(
        ...
        (sew === "b000".U) -> VecInit(
          (0 until bytesPerSlot).map(i => (baseAddr + indices_v(i)))
        ),
        // 16-bit data. Each 2-byte element has an offset.
        (sew === "b001".U) -> VecInit(
          (0 until bytesPerSlot).map(i =>
            (baseAddr + indices_v(i >> 1) + (i & 1).U)
          )
        ),
        // 32-bit data. Each 4-byte element has an offset.
        (sew === "b010".U) -> VecInit(
          (0 until bytesPerSlot).map(i =>
            (baseAddr + indices_v(i >> 2) + (i & 3).U)
          )
        )
      )
    )
...
class LsuSlot(p: Parameters, bytesPerSlot: Int) extends Bundle {
  ...
  def targetAddress(lastRead: Valid[UInt]): Valid[UInt] = {
    ...
    MuxCase(
      ...
      (0 until bytesPerSlot).map(i =>
        lineActive(i) -> MakeValid(!pendingVector(), addrs(i))
      )
    )
  }
  ...
  def vectorUpdate(rvv2lsu: Rvv2Lsu): LsuSlot = {
    ...
    val segmentBaseAddr =
      baseAddr + (segmentStride * vectorLoop.segment.curr)(31, 0)
    val bitsPerSlot     = bytesPerSlot * 8
    val indices         = MuxUpTo1H(
      rvv2lsu.idx.bits.data,
      Seq(
        // 2 of 2
        ((indexParitions === 2.U) && (vectorLoop.lmul
          .curr(0) === 1.U))    -> (rvv2lsu.idx.bits
          .data(bitsPerSlot - 1, bitsPerSlot / 2)),
        // 2 of 4
        ((indexParitions === 4.U) && (vectorLoop.lmul
          .curr(1, 0) === 1.U)) -> (rvv2lsu.idx.bits
          .data(bitsPerSlot / 2 - 1, bitsPerSlot / 4)),
        // 3 of 4
        ((indexParitions === 4.U) && (vectorLoop.lmul
          .curr(1, 0) === 2.U)) -> (rvv2lsu.idx.bits
          .data(bitsPerSlot * 3 / 4 - 1, bitsPerSlot / 2)),
        // 4 of 4
        ((indexParitions === 4.U) && (vectorLoop.lmul
          .curr(1, 0) === 3.U)) -> (rvv2lsu.idx.bits
    ...
    result.active := VecInit.tabulate(bytesPerSlot)(i =>
      active(i) || newActiveBytes(i)
    )
  ...
  def loadUpdate(lineAddr: UInt, lineData: UInt): LsuSlot = {
    ...
    result.data             := VecInit(
      (0 until bytesPerSlot).map(i =>
        Mux(lineActive(i), gatheredData(i), data(i))
      )
    )
    result.elemStride       := elemStride
    result.segmentStride    := segmentStride
  ...
  def fromLsuUOp(uop: LsuUOp, p: Parameters, bytesPerSlot: Int): LsuSlot = {
    ...
    val active = MuxUpTo1H(
      ...
      Seq(
        ...
        uop.op.isOneOf(LsuOp.LH, LsuOp.LHU, LsuOp.SH)   -> "b11".U(
          bytesPerSlot.W
        ),
        uop.op.isOneOf(LsuOp.LW, LsuOp.SW, LsuOp.FLOAT) -> "b1111".U(
          bytesPerSlot.W
        ),
        // Vector
    ...
    result.addrs     := Mux(
      ...
      ComputeStridedAddrs(
        ...
        uop.addr,
        uop.data,
        uop.elemWidth.getOrElse(0.U(3.W))
      ),
...
class LsuV2(p: Parameters) extends Lsu(p) {
  ...
  val opQueue = Module(
    new CircularBufferMulti(new LsuUOp(p), p.instructionLanes, 4)
  )
  ...
  val validSum   =
    io.req.map(_.valid).scan(0.U(log2Ceil(p.instructionLanes + 1).W))(_ + _)
  ...
  val targetLine     =
    MakeValid(targetAddress.valid, targetAddress.bits(31, nextSlot.elemBits))
  ...
  // ibus data path
  io.ibus.valid := loadUpdatedSlot
    .activeTransaction() && itcm && !slot.store && !faultReg.valid
  ...
  readFired := MakeValid(
    ...
    MuxCase(
      ...
      Seq(
        ...
        (ebusFired && !io.ebus.dbus.write) -> LsuRead(
          LsuBus.EXTERNAL,
          targetLine.bits
        )
      )
  ...
  val transactionUpdatedSlot =
    Mux(slot.store, slot.storeUpdate(storeUpdate), loadUpdatedSlot)
  ...
  io.rd.valid := ((faultReg.valid && LsuOp.isScalarLoad(
    faultReg.bits.op
  )) || slot.shouldWriteback()) &&
  ...
  // Float writeback
  io.rd_flt.valid     := ((faultReg.valid && !currentStore) || slot
    .shouldWriteback()) &&
  ...
  if (p.enableRvv) {
    ...
    io.lsu2rvv.get(0).valid     := (slot.shouldWriteback() && LsuOp.isVector(
      currentOp
    )) || vectorFault
    io.lsu2rvv.get(0).bits.addr := Mux(
      faultReg.valid,
      faultReg.bits.rd,
      slot.rd
    )
  ...
  opQueue.io.deqReady := Mux(
    slot.slotIdle() && (opQueue.io.nEnqueued > 0.U),
    1.U,
    0.U
  )
