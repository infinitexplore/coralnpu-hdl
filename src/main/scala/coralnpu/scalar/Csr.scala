...
class Csr(p: Parameters) extends Module {
  ...
  val tdata1    =
    Option.when(p.useDebugModule)(RegInit("x60000000".U.asTypeOf(new Tdata1)))
  ...
  val misa = RegInit(
    ((
      0x40001100 |
        (if (p.enableRvv) { 1 << 21 /* 'V' */ }
         else { 0 }) |
        (if (p.enableFloat) { 1 << 5 /* 'F' */ }
         else { 0 })
    ).U)(32.W)
  )
  ...
  // Debug CSRs.
  val tselectEn                        =
    Option.when(p.useDebugModule)(csr_address === CsrAddress.TSELECT)
  val tdata1En                         =
    Option.when(p.useDebugModule)(csr_address === CsrAddress.TDATA1)
  val tdata2En                         =
    Option.when(p.useDebugModule)(csr_address === CsrAddress.TDATA2)
  ...
  val dscratch0En                      =
    Option.when(p.useDebugModule)(csr_address === CsrAddress.DSCRATCH0)
  val dscratch1En                      =
    Option.when(p.useDebugModule)(csr_address === CsrAddress.DSCRATCH1)
  ...
  val is_csr_write = req.valid && !(req.bits.op
  ...
  if (p.useDebugModule) {
    val trigger_enabled = tdata1.get.isTrigger6 && tdata1.get.m
    val trigger_match   = trigger_enabled && io.dm.get.current_pc === tdata2.get

    val entering_debug_mode =
      (mode =/= CsrMode.Debug) && (io.dm.get.debug_req || trigger_match)
    val exiting_debug_mode  = (mode === CsrMode.Debug) && (io.dm.get.resume_req)
    mode                 := MuxCase(
      mode,
      Seq(
        entering_debug_mode  -> CsrMode.Debug,
        exiting_debug_mode   -> Mux(
          dcsr.get.prv === 3.U,
          CsrMode.Machine,
          CsrMode.User
        ),
    ...
    io.dm.get.debug_mode := (mode === CsrMode.Debug) || entering_debug_mode
    val newCause  = MuxCase(
      DebugCause.other,
      Seq(
        (io.dm.get.debug_req && !io.dm.get.dcsr_step) -> DebugCause.haltreq,
        trigger_match                                 -> DebugCause.trigger,
        io.dm.get.dcsr_step                           -> DebugCause.step
      )
    )
    dcsr.get := MuxCase(
      dcsr.get,
    ...
    val dpc_value =
      Mux(newCause === DebugCause.step, io.dm.get.next_pc, io.dm.get.current_pc)
    dpc.get            := MuxCase(
      dpc.get,
      Seq(
    ...
    io.dm.get.debug_pc := MuxCase(
      MakeInvalid(UInt(p.fetchAddrBits.W)),
      Seq(
        (req.valid && dpcEn.get && mode === CsrMode.Debug) -> MakeValid(wdata)
      )
    )
