...
class L1DCache(p: Parameters) extends Module {
  ...
  val preread = ~io.dbus.addr(
    11,
    linebit
  ) =/= 0.U && !io.dbus.write && !dempty // Within 4KB
  val addrA = Mux(
    io.dbus.addr(linebit),
    BankInAddress(io.dbus.adrx),
    BankInAddress(io.dbus.addr)
  )
  val addrB = Mux(
    io.dbus.addr(linebit),
    BankInAddress(io.dbus.addr),
  ...
  // Write masks
  val wmaskSA =
    ((~0.U(linebytes.W)) << io.dbus.addr(linebit - 1, 0))(linebytes - 1, 0)
  val wmaskSB =
    ((~0.U(linebytes.W)) >> (linebytes.U - io.dbus.addr(linebit - 1, 0)))(
      linebytes - 1,
      0
    )
  ...
  io.axi.write.data.bits  := Mux(
    waxi0,
    bank0.io.axi.write.data.bits,
    bank1.io.axi.write.data.bits
  )
  ...
  assert(
    !(io.axi.write.addr.valid && (io.axi.write.addr.ready =/= io.axi.write.data.ready))
  )
...
class L1DCacheBank(p: Parameters) extends Module {
  ...
  val io = IO(new Bundle {
    ...
    val axi      =
      new AxiMasterIO(p.axi1AddrBits - 1, p.axi1DataBits, p.axi1IdBits - 1)
    val flush    = Flipped(new DFlushIO(p))
    val volt_sel = Input(Bool())
  })
  io.axi.defaults()
  ...
  assert(
    assoc == 2 || assoc == 4 || assoc == 8 || assoc == 16 || assoc == slots
  )
  assert(
    assoc != 2 || setLsb == checkBit && setMsb == (checkBit + 6) && tagLsb == (checkBit + 7)
  )
  assert(
    assoc != 4 || setLsb == checkBit && setMsb == (checkBit + 5) && tagLsb == (checkBit + 6)
  )
  assert(
    assoc != 8 || setLsb == checkBit && setMsb == (checkBit + 4) && tagLsb == (checkBit + 5)
  )
  assert(
    assoc != 16 || setLsb == checkBit && setMsb == (checkBit + 3) && tagLsb == (checkBit + 4)
  )
  ...
  assert(
    !(io.dbus.valid && io.dbus.write) || (io.dbus.wmask & ~chkmask) === 0.U
  )
  ...
  for (i <- 0 until slots) {
    val set      = i / assoc
    val setMatch =
      if (assoc == slots) true.B else io.dbus.addr(setMsb, setLsb) === set.U
    matchSet(i) := setMatch
  }
  ...
  for (i <- 0 until slots / assoc) {
    ...
    when(
      io.dbus.valid && io.dbus.ready && (if (assoc == slots) true.B
                                           ...
                                           io.dbus.addr(setMsb, setLsb) === i.U)
    ) {
      for (j <- 0 until assoc) {
        when(matchSet(j)) {
  ...
  object FlushState extends ChiselEnum {
    val sNone, sCapture, sProcess, sMemwaddr, sMemwdata, sAxiready, sAxiresp,
        sEnd = Value
  }
  ...
  switch(fstate) {
    is(FlushState.sNone) {
      when(
        io.flush.valid && !axiwaddrvalid && !axiwdatavalid && !axiraddrvalid && !axirdataready
      ) {
  ...
  mem.io.wdata    :=
    Mux(
      ...
      Mem8to9(
        Cat(
          0.U((256 - io.axi.read.data.bits.data.getWidth).W),
          io.axi.read.data.bits.data
        ),
