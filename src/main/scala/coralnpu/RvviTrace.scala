...
object GenerateRvviTraceSource {
  def apply(p: Parameters): String = {
      ...
      for (j <- 0 until p.retirementBufferSize) {
        moduleInterface +=
          "  input logic [GENSZ:0] csr_i_GENIDX,\n"
            ...
            .replaceAll(
              "GENSZ",
              (((4096 / p.retirementBufferSize) * 32) - 1).toString
            )
      }
      moduleInterface +=
    ...
    var coreInstantiation = """
        ...
        |  ) rvvi();
        |"""
      .replaceAll("GEN_retirementBufferSize", p.retirementBufferSize.toString)
      .stripMargin
    ...
    for (i <- 0 until p.retirementBufferSize) {
      ...
      for (j <- 0 until p.retirementBufferSize) {
        coreInstantiation += "  assign rvvi.csr[0][GENi][MSB:LSB] = csr_i_GENIDX;\n"
          ...
          .replaceAll(
            "MSB",
            ((j + 1) * (4096 / p.retirementBufferSize) - 1).toString
          )
...
class RvviTrace(p: Parameters) extends Module {
  ...
  for (i <- 0 until p.retirementBufferSize) {
    ...
    for (j <- 0 until p.retirementBufferSize) {
      ...
      rvviTraceBlackBox.io.csr_i(i * p.retirementBufferSize + j) := csr(i)
        .asUInt(subsize * (j + 1) - 1, subsize * j)
    }
