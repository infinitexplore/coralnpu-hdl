...
class CoreTlul(p: Parameters, coreModuleName: String) extends RawModule {
  ...
  val coreAxi      = withClockAndReset(io.clk, io.rst_ni) {
    Module(new CoreAxi(p, coreModuleName))
  }
  ...
  val deviceBridge =
    withClockAndReset(io.clk, (!io.rst_ni.asBool).asAsyncReset) {
      Module(
        new TLUL2Axi(
          p,
          p,
