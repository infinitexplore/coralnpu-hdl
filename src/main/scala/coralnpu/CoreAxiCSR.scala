...
class CoreCSR(p: Parameters) extends Module {
  ...
  val rsp_queue = if (p.useDebugModule) {
    ...
    req_valid_pulse        := Mux(
      write_to_op_reg && io.debug.get.req.ready,
      true.B,
      false.B
    )
  ...
  pcStartReg := Mux(
    writeEn && writeAddr === 0x4.U,
    writeData(63, 32),
    pcStartReg
  )
  if (p.useDebugModule) {
    debugReqAddrReg.get := Mux(
      writeEn && writeAddr === CoreCsrAddrs.DbgReqAddr,
      writeData(31, 0),
      debugReqAddrReg.get
    )
    debugReqDataReg.get := Mux(
      writeEn && writeAddr === CoreCsrAddrs.DbgReqData,
      writeData(63, 32),
      debugReqDataReg.get
    )
    debugReqOpReg.get   := Mux(
