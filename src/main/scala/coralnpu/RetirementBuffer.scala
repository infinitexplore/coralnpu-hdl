...
class RetirementBufferIO(p: Parameters) extends Bundle {
  ...
  val writeDataScalar = Input(
    Vec(p.instructionLanes + 2, Valid(new RegfileWriteDataIO))
  )
  val writeAddrFloat  = Option.when(p.enableFloat)(Input(new RegfileWriteAddrIO))
  val writeDataFloat  =
    Option.when(p.enableFloat)(Input(Vec(2, Valid(new RegfileWriteDataIO))))
  val writeAddrVector = Option.when(p.enableRvv)(
    Input(Vec(p.instructionLanes, new RegfileWriteAddrIO))
  )
  val writeDataVector = Option.when(p.enableRvv)(
    Input(Vec(p.instructionLanes, Valid(new VectorWriteDataIO(p))))
  )
...
/**
 *  The Retirement Buffer manages the lifecycle of instructions from dispatch to
 *  retirement.
 *
 *  Instruction Lifecycle:
 *    - Dispatched: The instruction is enqueued into the buffer upon dispatch.
 *    - Completed: All side effects (register writes, store completions, or
 *      faults) are committed to the architectural state.
 *    - Retired: The final state. Instructions are dequeued from the buffer
 *      in-order when they and all preceding instructions are completed.
 */
class RetirementBuffer(p: Parameters, mini: Boolean = false) extends Module {
  ...
  val seenFalseV =
    (instFires.scanLeft(false.B) { (acc, curr) => acc || !curr }).drop(1)
  assert(
    !(seenFalseV
      .zip(instFires)
      .map({ case (seenFalse, fire) => seenFalse && fire })
      .reduce(_ | _))
  )
  ...
  def fault(
    ...
    instr.inst   := Mux(
      !mini.B && io.fault.bits.mcause === 2.U,
      io.fault.bits.mtval,
      finst.inst
    )
  ...
  val insts = (0 until p.instructionLanes).map(i => {
    ...
    val linkOk = if (i == 0) {
      (io.inst(0).bits.addr === regLastTarget) || (regLastIsBranch && io
        .inst(0)
        .bits
        .addr === regLastAddr + 4.U)
    } else {
  ...
  regLastTarget   := Mux(
    hasFire,
    PriorityMux(instFires.reverse, targetsList.reverse),
    regLastTarget
  )
  regLastAddr     := Mux(
    hasFire,
    PriorityMux(instFires.reverse, addrList.reverse),
    regLastAddr
  )
  regLastIsBranch := Mux(
    hasFire,
  ...
  val vectorWriteAccumulator = Option.when(!mini && p.enableRvv)(
    RegInit(
      VecInit.fill(bufferSize)(
        VecInit.fill(8)(0.U.asTypeOf(Valid(new VectorWrite)))
      )
    )
  )
  ...
  for (i <- 0 until bufferSize) {
    ...
    if (!mini && p.enableRvv) {
      ...
      for (k <- 0 until 8) {
        ...
        nextEntry(k).valid     := Mux(
          anyHit,
          true.B,
          vectorWriteAccumulator.get(pIdx)(k).valid
        )
      ...
      vectorAccumulatorNext.get(pIdx) := Mux(
        validBufferEntry,
        nextEntry,
        vectorWriteAccumulator.get(pIdx)
      )
      debugVectorWrites.get(pIdx)     := Mux(
        validBufferEntry,
        nextEntry,
        vectorWriteAccumulator.get(pIdx)
      )
    ...
    val nextValid     =
      if (i < bufferSize - 1) ((i.U +& 1.U) < instBuffer.io.nEnqueued)
      else false.B
    val nextAddr      =
      if (i < bufferSize - 1) instBuffer.io.dataOut(i + 1).addr else 0.U
    ...
    val nextLinkOk =
      if (i < bufferSize - 1) instBuffer.io.dataOut(i + 1).linkOk else true.B
    ...
    val cfMatch =
      nextAddrValid && (targetMatch || (isBranch && fallthroughMatch))
    ...
    val newDataDone =
      validBufferEntry && !prevDataDone && (dataReady || currentTrap)
    ...
    if (!mini) {
      ...
      val writeDataFloat  =
        io.writeDataFloat.map(x => x(floatWriteIdx).bits.data).getOrElse(0.U)
      val writeDataVector =
        io.writeDataVector.map(x => x(vectorWriteIdx).bits.data).getOrElse(0.U)

      // Select the correct write-back data to store, if updated (FP has priority).
      val sdata =
        if (p.enableRvv) Cat(0.U((p.lsuDataBits - 32).W), writeDataScalar)
        else writeDataScalar
      val fdata =
        if (p.enableRvv) Cat(0.U((p.lsuDataBits - 32).W), writeDataFloat)
        else writeDataFloat
      ...
      val allowWritebackTrap =
        validBufferEntry && isControlFlow && newCfDone && noFire0Fault
      resultUpdate(i).bits.result := Mux(
        currentTrap && !allowWritebackTrap,
        0.U,
        result
      )
    }
  ...
  val countValid   = Cto(
    VecInit(resultUpdate.map(x => x.valid && x.bits.cfDone)).asUInt
  )
  ...
  val retiredEcalls = PopCount(
    VecInit(
      (0 until bufferSize).map(i =>
        (i.U < deqReady) && instBuffer.io.dataOut(i).isEcall
      )
    ).asUInt
  )
  ...
  for (i <- 0 until bufferSize) {
    ...
    val allowDebug = resultUpdate(i).bits.trap && instBuffer.io
      .dataOut(i)
      .isControlFlow && noFire0Fault
    io.debug.inst(i).valid     := valid
    io.debug.inst(i).bits.pc   := MuxOR(valid, instBuffer.io.dataOut(i).addr)
    io.debug.inst(i).bits.inst := MuxOR(
      valid && !mini.B,
      instBuffer.io.dataOut(i).inst
    )
    io.debug.inst(i).bits.data := MuxOR(
      valid && !mini.B,
      resultUpdate(i).bits.result
    )
    io.debug.inst(i).bits.idx  := MuxOR(
      valid,
      Mux(
        resultUpdate(i).bits.trap && !allowDebug,
        noWriteRegIdx,
        instBuffer.io.dataOut(i).idx
      )
    )
    ...
    if (!mini && p.enableRvv) {
      ...
      io.debug.inst(i).bits.vecWrites.get := 0.U.asTypeOf(
        io.debug.inst(i).bits.vecWrites.get
      )
    }
  }
}
