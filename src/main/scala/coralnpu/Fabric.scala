...
class FabricArbiter(p: Parameters, n: Int = 2) extends Module {
  ...
  assert(
    !io.source
      .map(x => x.readDataAddr.valid && x.writeDataAddr.valid)
      .reduce(_ || _)
  )
  val sourceValid =
    io.source.map(x => x.readDataAddr.valid || x.writeDataAddr.valid)
...
class FabricMux(p: Parameters, regions: Seq[MemoryRegion]) extends Module {
  ...
  val portSelected = (0 until portCount).map(i =>
    selected.valid && (selected.bits === i.U) && !io.periBusy(i)
  )
