...
class MemoryRegion(
  ...
  def contains(addr: UInt): Bool = {
    ...
    (addr >= memStart
      .U(addrWidth)) && (addr < memStart.U(addrWidth) + memSize.U(addrWidth))
  }
...
object MemoryRegions {
  ...
  def highmem(itcmSizeKBytes: Int, dtcmSizeKBytes: Int) = Seq(
    ...
    new MemoryRegion(
      0x00000000,
      itcmSizeKBytes * 1024,
      MemoryRegionType.IMEM
    ),                                                                    // ITCM
    new MemoryRegion(
      0x00100000,
      dtcmSizeKBytes * 1024,
      MemoryRegionType.DMEM
    ),                                                                    // DTCM
...
class Parameters(var m: Seq[MemoryRegion] = Seq(), val hartId: Int = 0) {
  ...
  def retirementBufferIdxWidth: Int = {
    ...
    val floatRegCount  = (if (enableFloat) { 32 }
                         else { 0 })
...
object EmitParametersHeader {
  def apply(p: Parameters): String = {
    ...
    builder = builder.append(
      s"#define KP_useRetirementBuffer ${p.useRetirementBuffer}\n"
    )
    builder = builder.append(
      s"#define KP_retirementBufferIdxWidth ${p.retirementBufferIdxWidth}\n"
    )
