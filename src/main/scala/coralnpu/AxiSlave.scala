...
class AxiSlave(p: Parameters) extends Module {
  ...
  val addrArbiter = Module(
    new CoralNPURRArbiter(
      new AxiAddress(p.axi2AddrBits, p.axi2DataBits, p.axi2IdBits),
      2
    )
  )
  ...
  readIssued  := MakeValid(
    issueRead,
    ReadResult(p.axi2IdBits, axiAddrCmd.bits.addr.id, lastRead)
  )
  ...
  val baseAddrMask        = VecInit(
    (0 until axiAddrCmd.bits.addr.addr.getWidth).map(x =>
      !(x.U < axiAddrCmd.bits.addr.size)
    )
  )
