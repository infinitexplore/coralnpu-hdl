 ...
 *    The bit-width of the id field in the AXI Interface.
 *  @param reads
 *    A map of write names to target address. Returns a map of names->written as
 *    well as the data written.
 */
object ConnectAxiWrite {
  def apply(
    ...
    axiWrite.resp.bits.resp := Mux(
      writes.values.toSeq.reduce(_ || _),
      0.U(2.W),
      "b10".asUInt(2.W)
    )
